...
class Inventory:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self, "min_stock_level", self._round_quantity(self.min_stock_level)
        )
        if self.max_stock_level is not None:
            object.__setattr__(
                self, "max_stock_level", self._round_quantity(self.max_stock_level)
            )
        object.__setattr__(
            self, "reorder_point", self._round_quantity(self.reorder_point)
        )
        object.__setattr__(
            self, "reorder_quantity", self._round_quantity(self.reorder_quantity)
        )
    ...
    def from_db_row(cls, row: Dict[str, Any]) -> "Inventory":
        ...
        try:
            return cls(
                ...
                max_stock_level=(
                    float(str(row["max_stock_level"]))
                    if row.get("max_stock_level") is not None
                    else None
                ),
    ...
    def get_suggested_order_quantity(self) -> Optional[float]:
        ...
        if self.max_stock_level is not None:
            return min(self.reorder_quantity, self.max_stock_level - self.quantity)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "max_stock_level": (
                str(self.max_stock_level) if self.max_stock_level is not None else None
            ),
