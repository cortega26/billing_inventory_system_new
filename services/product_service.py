    ...
    def update_product(product_id: int, name: str, description: Optional[str], category_id: Optional[int]) -> None:
        try:
            ...
            if category_id is not None:
                ...
                category_id = current_product.category.id if current_product.category else None
            ...
            logger.info(f"  Name: {current_product.name} -> {name}")
            logger.info(f"  Description: {current_product.description} -> {description}")
            logger.info(f"  Category ID: {current_product.category.id if current_product.category else None} -> {category_id}")
