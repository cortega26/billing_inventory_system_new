...
class TestCustomerService:
    ...
    def test_create_customer_invalid_identifier(self, customer_service):
        with pytest.raises(ValidationException):
            customer_service.create_customer(
                identifier_9="12345",  # Invalid length
                name="Test Customer"
            )

    def test_create_customer_duplicate_identifier(self, customer_service, sample_customer_data, mocker):
        # Mock first customer creation success
        mock_execute_success = mocker.patch('database.database_manager.DatabaseManager.execute_query')
        ...
        mock_execute_error = mocker.patch(
            'database.database_manager.DatabaseManager.execute_query',
            side_effect=ValidationException("Duplicate identifier")
        )
    ...
    def test_delete_customer(self, customer_service, sample_customer_data, mocker):
        ...
        mock_customer.id = 1
        mocker.patch.object(customer_service, 'get_customer', side_effect=[mock_customer, None])
        ...
        with pytest.raises(NotFoundException):
            customer_service.get_customer(1)
    ...
    def test_get_customer_purchase_history(self, customer_service, sample_customer_data):
        customer_id = customer_service.create_customer(
            identifier_9=sample_customer_data["identifier_9"],
            name=sample_customer_data["name"]
        )
    ...
    def test_validate_identifiers(self, customer_service):
        ...
        # Valid identifiers
        assert customer_service.validate_identifier("123456789", "identifier_9") == "123456789"
        ...
        # Invalid identifiers
        with pytest.raises(ValidationException):
            customer_service.validate_identifier("12345678", "identifier_9")  # Too short
        with pytest.raises(ValidationException):
