    ...
    def setup_ui(self):
        ...
        for customer in self.customers:
            self.customer_combo.addItem(
                f"{customer.identifier_9} ({customer.identifier_3or4 or 'N/A'})",
                customer.id,
            )
...
class SaleView(QWidget):
    ...
    def setup_ui(self):
        ...
        self.sale_table = create_table(
            ["ID", "Customer", "Date", "Total Amount", "Actions"]
        )
    ...
    def update_sale_table(self, sales: List[Sale]):
        ...
        for row, sale in enumerate(sales):
            ...
            self.sale_table.setItem(
                row, 2, QTableWidgetItem(sale.date.strftime("%Y-%m-%d"))
            )

            self.sale_table.setItem(
                row, 3, PriceTableWidgetItem(sale.total_amount, format_price)
            )
    ...
    def add_sale(self):
        ...
        if items:
            sale_id = self.sale_service.create_sale(
                self.selected_customer_id, date, items
            )
    ...
    def view_sale(self, sale: Sale):
        ...
        message += f"{'':=^64}\n\n"
        message += (
            f"{'Producto':<30}{'Cantidad':>10}{'P.Unitario':>12}{'Subtotal':>12}\n"
        )
        message += f"{'':-^64}\n"
        for item in items:
