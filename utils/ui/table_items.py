...
class PercentageTableWidgetItem(QTableWidgetItem):
    def __init__(self, value: Union[int, float, None]):
        display_value = f"{value:.2f}%".replace('.', ',') if value is not None else "N/A"
        super().__init__(display_value)
        self.value = value if value is not None else float('-inf')  # Use -inf for N/A to sort it at the bottom
        self.setTextAlignment(Qt.AlignmentFlag.AlignRight | Qt.AlignmentFlag.AlignVCenter)
...
class PriceTableWidgetItem(QTableWidgetItem):
    def __init__(self, value: Union[int, float, None], format_func: Callable[[Union[int, float]], str]):
